...
public class CustomizedPredicateBuilder {

    private static final Logger log = Logger.getLogger(CustomizedPredicateBuilder.class);
    ...
    static {
        //
        // Use a date range that Oracle can cope with - apparently the years around 1 BC and 1 AD are messed up in Oracle -
        // known bug
        //
        Calendar cal = Calendar.getInstance();
        cal.set(9999, Calendar.DECEMBER, 31);
        END_DATE = cal.getTime();
        cal.set(5, Calendar.JANUARY, 1); // Use Jan 1, 5 AD, since that's where the Roman's sort of got it together with leap
                                         // years.
     ...
     * Private constructor.
     */
    public CustomizedPredicateBuilder() {
        super();
    }
    ...
    public static <T> Predicate createPredicate(Node node, From root, Class<T> entity, EntityManager manager,
            BuilderTools misc) {
        log.infof("Creating Predicate for: %s", node);
    ...
    /**
     * + * Creates the between than. + * + * @param propertyPath the property path + * @param startDate the start date +
     * * @param argument the argument + * @param manager the manager + * @return the predicate +
     */
    ...
    /**
     * Apply a case-insensitive "like" constraint to the property path. Value should contains wildcards "*" (% in SQL) and "_".
     *
     * @param propertyPath Property path that we want to compare.
    ...
    /**
     * Apply a negative case-insensitive "like" constraint to the property path. Value should contains wildcards "*" (% in SQL)
     * and "_".
     *
     * @param propertyPath Property path that we want to compare.
     * @param argument Argument with/without wildcards
     ...
     * @return Predicate a predicate representation.
     */
    private static <Y extends Comparable<? super Y>> Predicate createGreaterThanComparable(Expression<? extends Y> propertyPath,
            Y argument, EntityManager manager) {
        CriteriaBuilder builder = manager.getCriteriaBuilder();
     ...
     * @return Predicate a predicate representation.
     */
    private static <Y extends Comparable<? super Y>> Predicate createLessThanComparable(Expression<? extends Y> propertyPath,
            Y argument, EntityManager manager) {
        CriteriaBuilder builder = manager.getCriteriaBuilder();
     ...
     * @return Predicate a predicate representation.
     */
    private static <Y extends Comparable<? super Y>> Predicate createLessEqualComparable(Expression<? extends Y> propertyPath,
            Y argument, EntityManager manager) {
        CriteriaBuilder builder = manager.getCriteriaBuilder();
     ...
     * @return Class java type for the property, if the property is a pluralAttribute it will take the bindable java type of
     *         that collection.
     */
    ...
    private static String buildNotComparableMessage(ComparisonOperator operator, Object argument) {
        return String.format("Invalid type for comparison operator: %s type: %s must implement Comparable<%s>", operator,
                argument.getClass().getName(), argument.getClass().getSimpleName());
    }
}